...
%% MLE
Z1=[const,L,month];
Z2=[const,month,DM]; %Supply exogenous variables without po
Y=[lnQ,lngr];
save 'Data/jecnew0' Y Z1 Z2 t T G lngr lnQ

load 'Data/param' % (from the 2SLS estimations)
param(33)=[]; %Drop array of po coefficients
beta=param;
nb=length(beta); %Total number of parameters
nb1d=size(Z1,2); %Number of exogenous demand variables
nb1s=size(Z2,2); %Number of exogenous pricing Eq. variables

options=optimset('MaxFunEvals',1000);
[~,L0,~,~]= fminsearch('LL0',beta,options); 
f=-L0;
