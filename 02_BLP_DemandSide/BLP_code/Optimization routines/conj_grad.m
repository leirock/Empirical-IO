function  [X, info, perf,neval] = conj_grad(fun,par, x0, opts)
...
found = ng <= opts(4);
fprintf('--------------------------------------------------------------------------\n');
fprintf('  Iter    fcn.evals    fcn.value   g-norm      a*h-norm                   \n')
fprintf('--------------------------------------------------------------------------\n');
...
while  ~found
     ...
     %  Update  gamma  and  x
     if  opts(1) == 1
         %apply Fletcher-Reeves
         gam = gg/ggpr;
     else
         %apply Polak Ribiere
         gam = dot(g-gpr,g)/ggpr; 
     end
     x = x + alpha*h;   
     neval = neval + dval;
     if  Trace
           X(:,k) = x(:);   
           perf(1:2,k) = [f; ng];
           perf(3:5,k-1) = [alpha; dot(h,g); dval]; 
     end
     %  Check stopping criteria
     if  ng <= opts(4)
         found = 1;
     elseif (alpha*nh) <= opts(5)*(opts(5) + norm(x));
         found = 2;         
     elseif  neval >= kmax
         found = 3; 
     end     
...
%**************************************************************************
%  Auxiliary Function Evaluations
%**************************************************************************
