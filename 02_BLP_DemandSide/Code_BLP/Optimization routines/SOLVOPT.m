...
warn10='Function evaluations limit exceeded.';
...
if k==ld
  fprintf('Iteration   : %5i\t',k);
  fprintf('Func. Evals : %5i\t',options(10));   
  fprintf('Func. Value : %12.4f\t',f);
  fprintf('Step size   : %12.4f\t',dx);
  fprintf('Grad norm   : %12.4f\n',ng);
...
% FUNCTION EVALUATIONS LIMIT
      if(options(10)>mymaxfunevals)
          options(9)=-9; if trx, x=x'; end,
          if dispwarn, disp(wrnmes);  disp(warn10); end
          return
      end
% ----}
